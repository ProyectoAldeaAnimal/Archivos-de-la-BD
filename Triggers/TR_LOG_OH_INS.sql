...
DECLARE TEMP int;
SET TEMP= (SELECT MAX(ID_OFERTA_HOR)FROM OFERTA_HOR);
SET DATOSD= (SELECT CONCAT(CAST(NEW.ID_OFERTA_HOR AS CHAR),',',CAST(NEW.ID_CAL AS CHAR),',',CAST(NEW.ID_VET AS CHAR),',',NEW.ESTADO_AGENDAMIENTO,',',CAST(NEW.HORA_INICIO AS CHAR),',',CAST(NEW.HORA_TERMINO AS CHAR)));
SET INST= 'INSERT';
